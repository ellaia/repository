...
#df = pd.read_excel('D:\StatsHaouz\PP_PAY_MAD.xlsx', parse_dates=['DATE_CREATION', 'DATE_UPDATE_STATUT', 'DATE_EXPIRATION'], date_parser=date_parser)
#df = pd.read_excel('D:\StatsHaouz\PP_PAY_MAD.xlsx', parse_dates=['DATE_CREATION', 'DATE_UPDATE_STATUT', 'DATE_EXPIRATION'])

df = pd.read_excel('D:\StatsHaouz\PP_PAY_MAD.xlsx')
df['DATE_CREATION'] = pd.to_datetime(df['DATE_CREATION'], errors='coerce')
df['DATE_UPDATE_STATUT'] = pd.to_datetime(df['DATE_UPDATE_STATUT'], errors='coerce')
...
time_bins_1H = pd.date_range(start_date, end_date + pd.Timedelta(hours=1), freq='1H')
...
# Extract unique creation dates dynamically
unique_creation_dates = sorted_df['DATE_CREATION'].drop_duplicates().sort_values()

# Manually set the cumulative values based on the information provided (replace these with your actual data if needed)
...
final_extended_dates = list(unique_creation_dates) + [pd.Timestamp.now().normalize() + pd.Timedelta(days=1)]
