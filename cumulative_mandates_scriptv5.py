...
# Adding current time to ensure the curve goes up to the current time
current_time = pd.Timestamp(datetime.now())
creation_dates = sorted(creation_dates.tolist() + [current_time])

for date in creation_dates:
    total_created += created_count_by_date.get(date, 0)
...
# Tracé
fig, ax = plt.subplots()
ax.plot(df_filtered_paye['DATE_UPDATE_STATUT'], df_filtered_paye['Cumul_PAYE'], label='Evol. Mandats payés', color='r')
ax.plot(df_created['DATE_CREATION'], df_created['Cumul_Créés'], label='Evol. Mandats crées', color='g')

ax.set(xlabel='Temps', ylabel='Nombre Cumulatif',
